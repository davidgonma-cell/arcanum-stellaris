...
(function () {
  ...
  function renderCard3D(card, idx) {
    ...
    // Arte SVG enriquecido — mayores únicos, menores generados, fallback elegante
    let frontArt = '';
    let backArt = '';
    if (typeof ARCANUM_ART !== 'undefined') {
      backArt = ARCANUM_ART.BACK || '';
      if (card.arcana === 'major' && ARCANUM_ART.MAJORS[card.id]) {
        frontArt = ARCANUM_ART.MAJORS[card.id];
      } else if (card.arcana === 'minor' && typeof ARCANUM_ART.buildMinor === 'function') {
        frontArt = ARCANUM_ART.buildMinor(card);
      }
    }
    // Fallback: símbolo grande si no hay arte disponible
    if (!frontArt) {
      frontArt = `
        <div class="card-3d__art-circle">
          <span class="card-3d__art-symbol">${card.symbol || '✦'}</span>
        </div>
      `;
    }
    if (!backArt) {
      backArt = `<div class="card-3d__pattern"></div><div class="card-3d__sigil">✦</div>`;
    }

    return `
      <div class="${cls}" data-idx="${idx}" data-tilt
