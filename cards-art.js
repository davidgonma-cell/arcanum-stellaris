...
(function (root) {
  ...
  // con esa imagen. Si no, caemos al SVG generado abajo.
  const ASSET_BASE = 'assets/cards/';

  // Wrapper que devuelve markup con <img> apuntando al jpg extraído
  function imageWrapper(filename) {
    return `
      <div class="cardart cardart--image">
        <img src="${ASSET_BASE}${filename}" alt="" loading="lazy"
             onerror="this.parentElement.classList.add('cardart--missing')"/>
      </div>
    `;
  }

  // Map: cada arcano mayor a su archivo de imagen
  const MAJOR_IMAGES = {
    m00: 'm00.jpg', m01: 'm01.jpg', m02: 'm02.jpg', m03: 'm03.jpg',
    m04: 'm04.jpg', m05: 'm05.jpg', m06: 'm06.jpg', m07: 'm07.jpg',
    m08: 'm08.jpg', m09: 'm09.jpg', m10: 'm10.jpg', m11: 'm11.jpg',
    m12: 'm12.jpg', m13: 'm13.jpg', m14: 'm14.jpg', m15: 'm15.jpg',
    m16: 'm16.jpg', m17: 'm17.jpg', m18: 'm18.jpg', m19: 'm19.jpg',
    m20: 'm20.jpg', m21: 'm21.jpg'
  };

  // Para menores: fichero según suit + número.
  // La lámina del usuario tiene cartas 1-10. Las figuras (aprendiz/caballero/
  // reina/rey, números 11-14) caen al SVG generado.
  function minorImageFile(card) {
    if (!card || !card.suit || !card.number) return null;
    if (card.number > 10) return null; // figuras no incluidas en la lámina
    const num = card.number.toString().padStart(2, '0');
    return `${card.suit}-${num}.jpg`;
  }
  ...
  // Builder principal de menores: prefiere imagen real, fallback a SVG
  function buildMinor(card) {
    if (!card || !card.suit) return '';
    // Si el menor tiene archivo extraído de la lámina, lo usamos
    const file = minorImageFile(card);
    if (file) return imageWrapper(file);

    // Fallback: figuras de la corte (11-14) usan SVG generado
  ...
  // Builder de mayores: prefiere imagen real, fallback al SVG ya construido
  function buildMajor(card) {
    if (!card) return '';
    const file = MAJOR_IMAGES[card.id];
    if (file) return imageWrapper(file);
    return MAJORS[card.id] || '';
  }

  // ──────────────────────────────────────────────────────────────────
  // BACK ART — usa la imagen reverso.jpg de la lámina
  // ──────────────────────────────────────────────────────────────────
  const BACK_ART = imageWrapper('reverso.jpg');
