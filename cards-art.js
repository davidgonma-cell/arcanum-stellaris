...
(function (root) {
  ...
  // ──────────────────────────────────────────────────────────────────
  // FRAME — marco decorativo común a todas las cartas
  // ──────────────────────────────────────────────────────────────────
  function frame(content, opts = {}) {
    ...
    return `
      <svg class="cardart" viewBox="0 0 200 280" preserveAspectRatio="xMidYMid meet" aria-hidden="true">
        <defs>
          <linearGradient id="cardBg-${opts.id || 'd'}" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stop-color="${opts.bg1 || C.paper}"/>
            <stop offset="100%" stop-color="${opts.bg2 || C.cream}"/>
          </linearGradient>
          <radialGradient id="cardSpot-${opts.id || 'd'}" cx="50%" cy="40%" r="60%">
            <stop offset="0%" stop-color="${opts.spot || '#fff'}" stop-opacity="0.55"/>
            <stop offset="100%" stop-color="${opts.spot || '#fff'}" stop-opacity="0"/>
          </radialGradient>
        </defs>
        <!-- Fondo papel + halo central -->
        <rect width="200" height="280" fill="url(#cardBg-${opts.id || 'd'})"/>
        <ellipse cx="100" cy="120" rx="90" ry="100" fill="url(#cardSpot-${opts.id || 'd'})"/>
        ...
        <!-- Florones de esquina -->
        <g stroke="${tint}" stroke-width="0.6" fill="none" opacity="0.75" stroke-linecap="round">
          <!-- TL -->
        ...
        <!-- Escena central -->
        <g transform="translate(0, 22)">${content}</g>
      </svg>
    `;
  }
  ...
  // ──────────────────────────────────────────────────────────────────
  // BACK ART — patrón mandala animado para el reverso
  // ──────────────────────────────────────────────────────────────────
  const BACK_ART = `
    <svg class="cardback" viewBox="0 0 200 280" preserveAspectRatio="xMidYMid meet" aria-hidden="true">
      <defs>
        <linearGradient id="backBg" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stop-color="${C.cream}"/>
          <stop offset="100%" stop-color="${C.sand}"/>
        </linearGradient>
        <radialGradient id="backSpot" cx="50%" cy="50%" r="60%">
          <stop offset="0%" stop-color="#fff" stop-opacity="0.45"/>
          <stop offset="100%" stop-color="#fff" stop-opacity="0"/>
        </radialGradient>
      </defs>
      <rect width="200" height="280" fill="url(#backBg)"/>
      <ellipse cx="100" cy="140" rx="100" ry="120" fill="url(#backSpot)"/>

      <!-- Marco doble -->
      <rect x="8" y="8" width="184" height="264" fill="none" stroke="${C.line}" stroke-width="0.7" opacity="0.7"/>
      <rect x="12" y="12" width="176" height="256" fill="none" stroke="${C.line}" stroke-width="0.4" opacity="0.5"/>

      <!-- Esquinas decorativas -->
      <g stroke="${C.line}" stroke-width="0.6" fill="none" opacity="0.7" stroke-linecap="round">
        <path d="M14 22 Q14 14 22 14"/>
        <path d="M186 22 Q186 14 178 14"/>
        <path d="M14 258 Q14 266 22 266"/>
        <path d="M186 258 Q186 266 178 266"/>
      ...
      <g transform="translate(100, 140)">
        <circle r="62" fill="none" stroke="${C.gold}" stroke-width="0.6" opacity="0.7"/>
        <circle r="48" fill="none" stroke="${C.gold}" stroke-width="0.4" opacity="0.6"/>
        <circle r="32" fill="none" stroke="${C.line}" stroke-width="0.5" opacity="0.6"/>
        <circle r="18" fill="none" stroke="${C.accent}" stroke-width="0.7"/>

        <!-- 8 pétalos -->
        <g class="cardback__rotor">
          ${[0, 45, 90, 135, 180, 225, 270, 315].map(deg => `
            <g transform="rotate(${deg})">
              <path d="M0 -55 Q-10 -35 0 -22 Q10 -35 0 -55 Z"
                    fill="none" stroke="${C.line}" stroke-width="0.5" opacity="0.7"/>
              <circle cx="0" cy="-55" r="2" fill="${C.gold}"/>
            </g>
          `).join('')}
        </g>

        <!-- Estrella central -->
        <path d="M0 -10 L2.4 -3 L10 -3 L4 1.5 L6 9 L0 5 L-6 9 L-4 1.5 L-10 -3 L-2.4 -3 Z"
              fill="${C.accent}" opacity="0.85"/>
      </g>

      <!-- Texto "Arcanum" abajo -->
      <text x="100" y="252" text-anchor="middle"
            font-family="Fraunces, serif" font-size="9" font-style="italic"
            fill="${C.line}" opacity="0.7" letter-spacing="3">A R C A N U M</text>
    </svg>
  `;
